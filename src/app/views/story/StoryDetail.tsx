...
const StoryDetail = ({ route, navigation }: any) => {
  ...
  const handleNext = () => {
    if (item && item.position < item.totalStories - 1
    ...
    } else if (item && itemLive.current?.nextStory) {
      // Move to next story in the same user's stories
      getOneStory(itemLive.current.nextStory);
    } else {
      navigation.goBack();
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.progressContainer}>
        {Array(item?.totalStories).fill(0).map((_, index) => (
          <View key={index} style={styles.progressWrapper}>
            <Animated.View
              style={[
                styles.progress,
                {
                  width: index < (item?.position ?? 0)
                    ? '100%' 
      ...
      <View style={styles.header}>
        <View style={styles.userInfo}>
          <Image
            source={item && currentUser ? { uri: currentUser.avatarUrl } : defaultUserImg}
            style={styles.avatar}
          />
          <Text style={styles.username}>{currentUser && currentUser.displayName}</Text>
        </View>
      ...
      >
        ...
        <Image
          source={item && item.imageUrl ? { uri: item.imageUrl } : avatarDefault}
